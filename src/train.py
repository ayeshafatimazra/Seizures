...
def _metrics(y_true, y_prob):
    ...
    return {"auc": auc, "auprc": auprc, "sensitivity": sens, "specificity": spec,
...
def cross_validate(X, y, groups, n_splits=None, models=None):
    ...
    for name, model in chosen.items():
        ...
        agg = {k: float(np.nanmean([m[k] for m in fold_metrics]))
               for k in ["auc", "auprc", "sensitivity", "specificity", "f1"]} if fold_metrics else {}
        results[name] = {"folds": len(fold_metrics), **agg}

    print(f"\n{'model':10} {'AUC':>6} {'AUPRC':>6} {'Sens':>6} {'Spec':>6} {'F1':>6}"
          f"  (GroupKFold, {n_splits} folds)")
    print("-" * 54)
    for name, r in results.items():
        if r.get("auc") is not None and "auc" in r:
            print(f"{name:10} {r['auc']:6.3f} {r['auprc']:6.3f} {r['sensitivity']:6.3f} "
